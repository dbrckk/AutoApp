...
export async function orchestrateGeneration(
  ...
  let buildResult = await runBuildCheck(mergedFiles, input.buildMode || "virtual");
  ...
  if (!buildResult.ok) {
    for (let attempt = 1; attempt <= 5; attempt++) {
      ...
      buildResult = await runBuildCheck(mergedFiles, input.buildMode || "virtual");
...
async function runBuildCheck(files: VirtualFile[], buildMode: "virtual" | "real" | "none") {
  if (buildMode === "none") {
    return {
      ok: true,
      issues: [],
