        ...
        >
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {item.message ? <p className="mt-1 text-xs leading-5 text-zinc-300">{item.message}</p> : null}
            </div>
            <button onClick={() => app.dismissNotification(item.id)} className="rounded-full bg-white/10 px-2 py-1 text-[10px] font-black text-white active:scale-[0.96]">×</button>
          </div>
        </article>
      ))}
    </div>
