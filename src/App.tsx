...
const NAV: { id: WorkspaceTab; label: string; icon: string }[] = [
  { id: "overview", label: "Projects", icon: "PR" },
...
export default function App() {
  ...
  const health = useMemo(() => {
    if (!app.activeJob) return "No active project";
    if (app.activeJob.status === "running") return "Running";
    if (app.activeJob.status === "done") return "Completed";
    return app.activeJob.status;
  }, [app.activeJob]);
    ...
    <main className="app-bg text-white">
      <div className="mx-auto flex min-h-screen w-full max-w-[1540px] gap-0 px-3 py-3 lg:px-4">
        <aside className="hidden w-[260px] shrink-0 flex-col border-r border-white/10 pr-3 lg:flex">
          <Brand />
          <button onClick={app.handleStartAutonomous} disabled={app.busy} className="neon-button mt-6 min-h-12 rounded-2xl px-4 text-sm font-black text-white transition active:scale-[0.98] disabled:opacity-50">+ New Project</button>
          <nav className="mt-6 grid gap-1">
            {NAV.map((item) => {
              const active = tab === item.id;
              return (
                <button key={item.id} onClick={() => setTab(item.id)} className={`flex min-h-12 items-center gap-3 rounded-2xl px-4 text-left text-sm transition ${active ? "bg-white/10 text-white shadow-lg" : "text-slate-400 hover:bg-white/6 hover:text-white"}`}>
                  <span className="grid h-7 w-7 place-items-center rounded-xl border border-white/10 bg-white/[0.04] text-[10px] font-black">{item.icon}</span>
          ...
          <div className="mt-auto grid gap-3 pb-3">
            <div className="glass-panel rounded-3xl p-4"><p className="text-xs font-black text-white">Current status</p><p className="mt-1 line-clamp-2 text-xs leading-5 text-slate-400">{app.status}</p></div>
            <div className="glass-panel rounded-3xl p-4"><p className="text-xs font-black text-white">AutoApp OS</p><p className="mt-1 text-xs text-slate-400">Premium workspace</p></div>
          </div>
        </aside>
        <section className="min-w-0 flex-1 lg:pl-4">
          <header className="glass-panel sticky top-3 z-30 mb-4 rounded-[1.7rem] p-3">
            <div className="flex items-center justify-between gap-3">
              <div className="min-w-0"><p className="text-[11px] font-black uppercase tracking-[0.24em] text-violet-200">AutoApp</p><h1 className="truncate text-xl font-black tracking-tight text-white sm:text-2xl">{app.activeJob?.target || "Autonomous Projects"}</h1></div>
              <div className="hidden min-w-0 flex-1 items-center justify-center px-4 md:flex"><div className="input-premium flex h-11 w-full max-w-md items-center gap-2 rounded-2xl px-4"><span className="text-xs font-black text-slate-500">REPO</span><input value={app.githubRepo} onChange={(event) => app.setGithubRepo(event.target.value)} placeholder="GitHub repo: owner/repo" className="w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-600" /></div></div>
              <div className="flex items-center gap-2"><button onClick={app.handleLiveDiagnostics} disabled={app.busy} className="hidden h-11 rounded-2xl border border-white/10 bg-white/[0.04] px-4 text-xs font-black text-white transition hover:bg-white/10 disabled:opacity-50 sm:block">Test</button><button onClick={app.handleStartAutonomous} disabled={app.busy} className="neon-button h-11 rounded-2xl px-4 text-xs font-black text-white disabled:opacity-50">New</button></div>
            </div>
          ...
          <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
            <div className="min-w-0 space-y-4">
              ...
              {tab === "overview" ? (<><ProjectsPanel app={app} /><ProfessionalPrompt app={app} /></>) : null}
...
function ProfessionalPrompt({ app }: { app: ReturnType<typeof useAutoApp> }) { return (<section className="glass-panel rounded-[2rem] p-5"><div className="mb-4"><h2 className="text-lg font-black text-white">Create professional project</h2><p className="mt-1 text-sm text-slate-500">The prompt is automatically strengthened with production requirements.</p></div><textarea value={app.prompt} onChange={(event) => app.setPrompt(event.target.value)} className="input-premium min-h-[220px] w-full rounded-3xl p-4 text-sm leading-6" /><div className="mt-4 grid gap-2 sm:grid-cols-2"><button onClick={app.handleGenerate} disabled={app.busy} className="min-h-12 rounded-2xl border border-white/10 bg-white/[0.06] px-4 text-sm font-black text-white disabled:opacity-50">Generate preview</button><button onClick={app.handleStartAutonomous} disabled={app.busy} className="neon-button min-h-12 rounded-2xl px-4 text-sm font-black text-white disabled:opacity-50">Start autonomous build</button></div></section>); }
