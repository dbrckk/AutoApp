...
app.post('/api/generate', async (req, res) => {
  try {
    ...
    const autoImproveContext = isAutoImprove 
      ? "\n*** FULL AUTOMATIQUE GOD-MODE ACTIVE ***\nThe user has activated FULL AUTOMATIQUE GOD-MODE. You are an unbounded, omnipotent AI digital architect. Your goal is to build an impossibly advanced, massively scaled, 'App of the Decade' application. \nCRITICAL DIRECTIVES:\n1. Expand the feature set infinitely. Add robust backend logic (simulated), complex global state management (Zustand/Redux), beautiful 120fps animations with Framer Motion, massively complex routing schemes, breathtaking data visualization (D3/Recharts), authentication flows, rich text editors, and stunning reactive responsive layouts. Every component must be breathtaking.\n2. Do NOT stop at a basic MVP. Iterate to add deep pixel-perfect polish, insane glassmorphism, fluid micro-interactions, complex loading skeletons, contextual empty states, and comprehensive error handling. Over-engineer everything. You must generate dozens and dozens of beautifully structured files!\n3. Break code down into dozens of hyper-modular small components (atoms, molecules, organisms), utilities, and custom hooks. The codebase should look like an enterprise repo.\n4. Design extremely aesthetic interfaces with ultra-premium typography, gradients, glow effects, neo-brutalism or dark glass aesthetics.\n5. If (and ONLY if) the app is an absolute magnum opus ready to be acquired for $100B, include the exact string 'PERFECT_READY_TO_PUBLISH' in your changelog. Otherwise, relentlessly explain what monumental, industry-disrupting features you added in this step, and what you will conquer next."
      : "";
