...
import { ensureAppSchema, inspectAppSchema } from "../core/schema";
...
const schemaInspection = await inspectAppSchema(c.env);
...
d1.status === "connected" && schemaInspection.ok
...
"GET /api/github/history?repo=OWNER/REPO",
...
diagnosticsRoutes.get("/schema", async (c) => {

const schema = await ensureAppSchema(c.env);

const inspection = await inspectAppSchema(c.env);

return c.json({

ok: Boolean(schema.ok && inspection.ok),
