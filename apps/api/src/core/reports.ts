...
export function createAutonomousReport({
  ...
  const professionalFocus = chooseProfessionalFocus(quality);

  const pipelineContext = createProfessionalPipelineContext({
    prompt: job.prompt,
    files,
  });
  ...
  return {
    ...
    professional: {
      ...
      qualityPrompt: qualityGateToPrompt(quality),
      productPlan: pipelineContext.productPlan,
      architecturePlan: pipelineContext.architecturePlan,
      designSystem: pipelineContext.designSystem,
    },
    ...
    capabilities: {
      ...
      professionalPipeline: "real_planner_architect_quality_gate_autofix",
      webApp: true,
    ...
    nextActions: buildProfessionalNextActions({
      score,
      build,
      quality,
      defaultActions: buildNextActions(score, build),
    }),
    ...
    summary: createReportSummary({
      ...
      readiness,
      focus,
      professionalScore: quality.total,
      professionalFocus,
      isAndroid,
      isGame,
      generatedAssets,
...
export function createReportSummary({
  ...
  if (typeof professionalScore === "number") {
    lines.push("Professional pipeline score: " + professionalScore + "/100.");
  }

  lines.push("Recommended next focus: " + focus + ".");

  if (professionalFocus) {
    lines.push("Professional focus: " + professionalFocus + ".");
  }
  ...
  if (infinite) {
    lines.push(
      "Infinite improvement is enabled; cron can continue after the browser is closed."
    );
  }
  ...
  if (isAndroid) {
    lines.push(
      "Android readiness score: " +
        (score.androidReady || 0) +
        "/100. APK/AAB build must happen outside Cloudflare Worker."
    );
  }

  if (!build.ok) {
    lines.push("Priority: repair build/import/dependency issues.");
  } else if ((professionalScore || score.total) < 90) {
    lines.push("Priority: continue autonomous improvement until professional score reaches 90+.");
  } else {
...
export function createPublishReport(files: VirtualFile[]) {
  ...
  const quality = runQualityGate(files);
  const blockers = [...inspection.missingCriticalFiles, ...quality.blockers];
  const buildRisks = [...(inspection.risks || []), ...quality.warnings];

  return {
    ready: blockers.length === 0 && score.total >= 75 && quality.total >= 75,
    score: Math.max(0, Math.min(score.total, quality.total) - blockers.length * 10),
    qualityScore: score.total,
    professionalScore: quality.total,
    scoreBreakdown: score,
    professionalBreakdown: quality.categories,
    blockers,
    warnings: buildRisks,
    nextFocus: chooseProfessionalFocus(quality),
    professional: {
      passed: quality.passed,
      blockers: quality.blockers,
      warnings: quality.warnings,
    ...
    checklist: [
      "Run professional pipeline quality gate.",
...
function buildProfessionalNextActions({
  score,
  build,
  quality,
  defaultActions,
  ...
  if (!build.ok) {
    actions.push("Repair build/import/dependency issues first.");
  }

  for (const blocker of quality.blockers) {
    actions.push("Fix blocker: " + blocker);
  }

  for (const suggestion of quality.suggestions.slice(0, 5)) {
    actions.push(suggestion);
  }

  if (quality.total < 75) {
    actions.push("Run professional autofix and another autonomous improve cycle.");
  }

  if (score.total < 85) {
    actions.push("Continue autonomous improvement until product score reaches 85+.");
  }

  return Array.from(new Set([...actions, ...defaultActions])).slice(0, 12);
}
